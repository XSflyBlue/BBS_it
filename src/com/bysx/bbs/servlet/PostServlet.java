...
import com.bysx.bbs.commons.util.ClientAccessIpUtil;
...
import com.bysx.bbs.domain.json.CollectionJson;
import com.bysx.bbs.domain.json.CommentJson;
import com.bysx.bbs.domain.json.PostFormJson;
import com.bysx.bbs.domain.json.PostJson;
...
import com.bysx.bbs.service.impl.AccessoryServiceImpl;
import com.bysx.bbs.service.impl.CollectionServiceImpl;
import com.bysx.bbs.service.impl.CommentServiceImpl;
import com.bysx.bbs.service.impl.ModeratorServiceImpl;
import com.bysx.bbs.service.impl.PostServiceImpl;
...
//		System.out.println(post);
		...
		if (post.getIsElite() != null && postFormList != null) {
			...
			for (int i = 0; i < postFormList.size(); i++) {
				if (postFormList.get(i) == null || postFormList.get(i).getRecomValidity() == null) {
					//忽略
				} else {
					long currentDate = new Date().getTime();
					long dbDate = postFormList.get(i).getRecomValidity().getTime();
...
//		System.out.println(post);
	...
	private void addPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			//判断用户是否登录
			if(userBase == null) {
				JSONUtils.writeJSON(response, new Msg(0, "未登录用户，发布帖子失败666"));
				return;
			}
...
//		System.out.println("updatePost-------------------");
		...
		String postType;     // 帖子类型
		String postContent;  // 帖子内容
		String accessoryStatus; // 帖子附件状态（新增0，删除1，重传2，更新帖子3）
		String sectionId;    // 板块Id
		...
		try {
			...
			if (post == null) {// 防止帖子被删
				...
				if(postContent==null||postContent.equals("")) {
					JSONUtils.writeJSON(response, new Msg(0, "帖子内容为空"));
					return;
				}
			}

			if(post != null&&post.getUserId().longValue()==userBase.getUserId().longValue()) {
				post.setPostType(Short.parseShort(postType));
		...
		if (accessoryStatus.equals("0")) {// 新增附件
			String message = (String) request.getAttribute("message");
			if (message != null && message.equals("文件上传成功！")) {
				Accessory accessory = new Accessory();
				accessory.setAuthor(UserServiceImpl.getInstance().findUserId(post.getUserId()).getUsername());
				...
				if(postFormList!=null) {
					...
					JSONUtils.writeJSON(response, new Msg(0, "帖子修改失败，附件上传失败"));
					return;
				}
			...
			post.setIsAccessory(Short.parseShort("1"));// 存在资源的帖子
			String message = (String) request.getAttribute("message");
			if (message != null && message.equals("文件上传成功！")) {
				Accessory accessory = new Accessory();
				accessory.setAuthor(UserServiceImpl.getInstance().findUserId(post.getUserId()).getUsername());
		...
		if (result == 0) {
			...
			JSONUtils.writeJSON(response, new Msg(1, "帖子更新成功"));
		}
	}
